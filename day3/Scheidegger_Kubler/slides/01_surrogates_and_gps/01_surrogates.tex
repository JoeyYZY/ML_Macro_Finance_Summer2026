...
\subtitle{Surrogate models for economics and finance --- with the Gaussian-process toolbox in the appendix}
...
\vspace{0.4em}
\textbf{Appendix --- the Gaussian-process toolbox} (the material of Lecture 6, Felix K\"ubler):
\begin{itemize}\itemsep2pt
  \item GP regression and deep kernels
  \item Bayesian active learning
  \item GPs for dynamic programming; active subspaces (up to 500D)
\end{itemize}

\vspace{0.3em}
\begin{center}
{\scriptsize Hands-on: \texttt{01\_01\_Surrogate\_Primer.ipynb} (main part); notebooks \texttt{01\_02}--\texttt{01\_09} accompany the appendix.}
\end{center}
\end{frame}
...
\end{itemize}
\vspace{0.3em}
{\small The Gaussian-process machinery behind several of these applications (GP regression, active learning, GP-based dynamic programming) is developed in the \emphc{appendix} and in notebooks \texttt{01\_02}--\texttt{01\_09}.}
\end{frame}
...
The appendix introduces \emphc{Gaussian Processes} and \emphc{Bayesian Active Learning} --- the tools behind this application (Lecture 6).
\end{alertblock}
\end{frame}
...
\begin{frame}{Pitfall: Lazy Learning --- Low Residual, Wrong Policy}
...
\begin{recipe}[Remedy]
\footnotesize In the deep-UQ application of Friedl et al.\ (2023) --- revisited in Session 3 --- only \gemph{10--20 anchors} stabilize training, with leave-one-out agreement \gemph{within $1\%$}.
\end{recipe}
\end{columns}
\vspace*{\fill}
\end{frame}
...
\textbf{Notebook:} \texttt{01\_01\_Surrogate\_Primer.ipynb} --- Black--Scholes surrogate with implied volatility inversion.
\end{block}
\end{frame}
...
\begin{center}
\textbf{Next up:}\\[6pt]
{\large Session 2:\\[2pt] Structural Estimation with SMM}\\[10pt]
...
{\normalsize A: GP regression \& deep kernels \,$\cdot$\, B: Bayesian active learning \,$\cdot$\, C: GPs for dynamic programming \& active subspaces}
\end{center}
\end{frame}
}
...
\smallskip
\textbf{Use cases.} Surrogate-health diagnostic across iterations of GP-VFI (Appendix C) and across active-learning rounds in SMM.
...
{\scriptsize See \textbf{Algorithm 2.1} in Rasmussen \& Williams (2006), and notebook \texttt{01\_02\_GP\_and\_BAL.ipynb}.}
\end{frame}
...
\vspace{0.5cm}
\begin{block}{Hands-on}
\textbf{Notebook:} \texttt{01\_02\_GP\_and\_BAL.ipynb} --- GP regression from scratch, scikit-learn, and Bayesian Active Learning.
\end{block}
\end{frame}
...
\emph{(Pre-generated illustration; notebook \texttt{01\_02\_GP\_and\_BAL.ipynb} reproduces the BAL loop itself.)}}
\end{frame}
...
\vspace{0.3cm}
\begin{block}{Hands-on}
\textbf{Notebook:} \texttt{01\_02\_GP\_and\_BAL.ipynb} --- GP from scratch, scikit-learn GP, and BAL comparison.
\end{block}
\end{frame}

% ============================================================================
% APPENDIX C: GPS FOR DYNAMIC PROGRAMMING & ACTIVE SUBSPACES
% ============================================================================
\section{Appendix C: GPs for Dynamic Programming \& Active Subspaces}

% --- Section Divider ---
\begin{frame}{}
\vfill
\begin{center}
{\Huge \textcolor{uzhblue}{\textbf{Appendix C}}}\\[12pt]
...
\[
(TV)(\x) := \max_\xi \bigl\{ r(\x, \xi) + \beta\, \E{V(\x_{t+1})} \bigr\}
\]
...
\vspace{0.3em}
\begin{block}{Hands-on}
\small \texttt{01\_03\_GP\_Value\_Function\_Iteration.ipynb} --- solves the $D=1$ case with GP-based VFI.
\end{block}
\end{frame}
...
\vspace{0.1em}
{\tiny \textbf{Hands-on:} \texttt{01\_04\_Active\_Subspace\_2D}, \texttt{01\_05\_..\_10D}, \texttt{01\_06\_..\_Nonlinear} --- AS construction and ASGP vs GP comparison.}
\end{frame}
...
\vspace{0.1em}
{\tiny \textbf{Hands-on:} \texttt{01\_08\_Deep\_Active\_Subspace\_Ridge} -- validates that deep AS recovers $d_{\mathrm{nl}} = 1$ on the radial ridge.}
\end{frame}
...
\vspace{0.1em}
{\tiny \textbf{Rule of thumb:} fit linear AS first; escalate to deep AS when (i) no gradients, (ii) ambiguous spectral gap, or (iii) $D \gg 10$ with curved manifold. \textbf{Hands-on:} \texttt{01\_08\_..\_Ridge} (curved wins), \texttt{01\_09\_..\_Borehole} (linear wins).}
\end{frame}
...
\vspace{0.4em}
\begin{block}{Hands-on}
\textbf{Notebook:} \texttt{01\_03\_GP\_Value\_Function\_Iteration.ipynb} --- solves the 1D growth model with GP-based VFI using scikit-learn. Shows convergence, value function with CI bands, and policy functions.
\end{block}
\end{frame}
...
\begin{tikzpicture}[
    ...
    \node[box, fill=softblue!15] (gp) at (0,2.5) {\textbf{Appendix A:} GP Fundamentals};
    \node[box, fill=softorange!15] (bal) at (0,1.25) {\textbf{Appendix B:} Bayesian Active Learning};
    \node[box, fill=softgreen!15] (dp) at (0,0) {\textbf{Appendix C:} GPs for Dynamic Programming};
...
\vspace{0.2em}
\begin{block}{Hands-on}
\small \textbf{Notebook:} \texttt{01\_03\_GP\_Value\_Function\_Iteration.ipynb} (in \texttt{code/01\_surrogates\_and\_gps/}).
\end{block}
\end{frame}

% ============================================================================
% APPENDIX D: TOOLBOX & REFERENCES
% ============================================================================
...
\vspace{0.3cm}
All four approaches are \emphc{complementary}: choose based on the problem structure, dimensionality, whether UQ is needed, and available hardware.\\[2pt]
...
\vspace{0.1cm}
{\tiny \textbf{Notebooks} (in \texttt{code/01\_surrogates\_and\_gps/}): \texttt{01\_01\_Surrogate\_Primer}, \texttt{01\_02\_GP\_and\_BAL}, \texttt{01\_03\_GP\_Value\_Function\_Iteration}, \texttt{01\_04--01\_06\_Active\_Subspace} (2D, 10D, nonlinear), \texttt{01\_07\_Deep\_Kernel\_Learning}, \texttt{01\_08\_Deep\_Active\_Subspace\_Ridge}, \texttt{01\_09\_Deep\_AS\_vs\_Linear\_AS\_Borehole}.  The SMM notebooks \texttt{02\_01} / \texttt{02\_02} accompany Session 2 (\texttt{code/02\_structural\_estimation\_smm/}).}
