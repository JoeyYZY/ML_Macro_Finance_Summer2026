...
\vspace{0.2em}
{\scriptsize \emphc{Notation:} in this session $\gamma$ is the \textbf{IES}; already this afternoon's OLG session re-purposes $\gamma$ for CRRA risk aversion (the switch is flagged there), as do later chapters of the script.}
...
\vspace{0.2em}
{\footnotesize $\sigma_e$ is the \emph{per-component} std., so the per-country innovation std.\ is $\sigma_e\sqrt{2}$ and the cross-country innovation correlation is $1/2$.}
...
\begin{enumerate}
    ...
    \item Open notebook \texttt{03\_01\_IRBC\_DEQN\_smooth.ipynb}, change $\delta$, and verify your prediction.
\end{enumerate}
\end{exampleblock}
\end{frame}
...
\begin{tikzpicture}[
    ...
    \node[mlstep] (nn) at (0, -2.0)
        {$\mathcal{N}_\theta : \R^{2N} \to \R^{2N+1}$};
...
\footnotesize
The loss is the \emphc{sum of squared residuals} across all equilibrium conditions; $\ell_\theta = 0$ iff every condition holds exactly.

\vspace{0.15em}
\textbf{Smooth benchmark} (notebook 03\_01):
\[
\boxed{
\ell^{\mathrm{smooth}}_\theta = \frac{1}{B}\sum_{i=1}^{B} \left[\sum_{j=1}^{N} \bigl(\text{Euler}_j^{(i)}\bigr)^2 + \bigl(\text{ARC}^{(i)}\bigr)^2\right]
}
\]

\vspace{0.1em}
\textbf{Irreversibility extension} (notebook 03\_02): add the Fischer--Burmeister block,
\[
\ell^{\mathrm{irrev}}_\theta = \ell^{\mathrm{smooth}}_\theta + \frac{1}{B}\sum_{i=1}^{B}\sum_{j=1}^{N}\bigl(\text{FB}_j^{(i)}\bigr)^2.
\]
...
\begin{itemize}\itemsep0pt
    ...
    \item $\text{FB}_j = \mu^j + I^j - \sqrt{(\mu^j)^2 + (I^j)^2 + \varepsilon^2}$ \quad (\emph{03\_02 only})
\end{itemize}
\end{frame}

% --- Slide 25: Gauss-Hermite Quadrature ---
\begin{frame}{Gauss--Hermite Quadrature (Recap)}
{\scriptsize\emph{Recap of Session 1's quadrature interlude.}}\vspace{-0.3em}
\footnotesize
...
\vspace{0.2em}
{\footnotesize \textbf{What the companion notebooks do.} Both 03\_01 and 03\_02 use the \emphc{Stroud-3 monomial rule} ($2(N+1)$ nodes; $6$ for $N=2$), not the tensor-product Gauss--Hermite above; the latter is shown for contrast and is practical only at very low $N$.}
\end{frame}
...
\begin{enumerate}
    ...
    \item \textbf{Current policy} $\mathcal{N}_\theta(\bm{s}_t) \to (k^{1\prime}, \ldots, k^{N\prime}, \lambda)$ \quad{\small(smooth, $N+1$ outputs; the irreversible extension appends $\mu^1, \ldots, \mu^N$)}
    ...
    \]
    \item \textbf{Next-period policy:} evaluate $\mathcal{N}_\theta(\bm{s}_{t+1,q})$
...
\begin{tikzpicture}[
    ...
    \node[mlstep] (nn) at (4.5,0) {$\mathcal{N}_\theta(\bm{s}_t)$};
...
\begin{algorithmic}
\footnotesize
\STATE \textbf{Input:} Network $\mathcal{N}_\theta$, learning rate $\eta$, segments $S$, monomial nodes/weights
\STATE Initialize $M$ trajectory heads $\bm{X}^{(1)}, \ldots, \bm{X}^{(M)}$ from a feasible starting cloud
\FOR{segment $s = 1, \ldots, S$}
    \STATE Simulate: for $t = 1, \ldots, T$, sample shocks and step each head one period under $\mathcal{N}_\theta$
    ...
    \FOR{pass $p = 1, \ldots, P$ \textbf{ over mini-batches of the segment}}
        \STATE Compute loss $\ell_\theta$ on the segment (smooth: Euler + ARC; irreversible: also FB)
...
\STATE \textbf{Output:} Trained $\mathcal{N}_{\theta^\star}$
\end{algorithmic}
\end{block}
\end{frame}
...
\begin{itemize}
    ...
    \item DEQN training cost per step: $\mathcal{O}(|\theta| \cdot B \cdot Q^{N+1})$
...
\centering
\small
$\mathcal{N}_\theta: \R^{2N} \to \R^{2N+1}$ with tanh hidden layers, linear output mapped through bounded transforms. All policies in one forward pass.
\end{block}
...
\footnotesize
\textbf{Three notebooks accompany this lecture} (all in \texttt{code/03\_irbc/}; self-contained, all equations inline):

\vspace{0.2em}
\begin{enumerate}\itemsep1pt
    \item \texttt{03\_01\_IRBC\_DEQN\_smooth.ipynb} -- \emph{smooth benchmark, in-class walk-through}
    \begin{itemize}\itemsep0pt
    ...
    \item \texttt{03\_02\_IRBC\_DEQN\_irreversible.ipynb} -- \emph{irreversible-investment extension}
    \begin{itemize}\itemsep0pt
    ...
    \item \texttt{03\_03\_IRBC\_Exercise.ipynb} -- \emph{hands-on exercise; uses a \emph{standalone} quarterly calibration ($\delta=0.025$, $A_{\mathrm{tfp}}=1$), deliberately decoupled from the DEQN notebooks' $k_{ss}=1$ normalization}
    \begin{itemize}\itemsep0pt
...
\begin{itemize}
    ...
    \begin{itemize}\itemsep1pt
        \item smooth (03\_01): $k^{j\prime} = k^j \exp\{\bar g \tanh r_j(\bm{s})\}$ keeps $k^{j\prime} > 0$;
        \item irreversible (03\_02): $k^{j\prime} = (1-\delta)k^j + I^j$ with $I^j \ge 0$ enforced via a sigmoid head.
    \end{itemize}
...
\begin{itemize}
    ...
    \item In a sensible SSS: $I^j \approx \delta\,k^j$ (investment replaces depreciation) and, in 03\_02, the irreversibility multiplier $\mu^j \approx 0$.
\end{itemize}
\end{frame}
...
\begin{enumerate}\itemsep3pt
    ...
    \item \textbf{Per-residual mean magnitudes} -- \texttt{mean\_abs\_euler}, \texttt{mean\_abs\_arc}, and (in 03\_02) \texttt{mean\_abs\_fb} -- all fall below the prescribed tolerance.
    ...
    \item \textbf{Zero-shock SSS} converges from dispersed feasible starts to a common point with $I^j \to \delta\,k^j$ and (in 03\_02) $\mu^j \to 0$.
\end{enumerate}

\vspace{0.4em}
{\footnotesize \emph{Historical note.}  An earlier version of this material compared four sampling/weighting approaches (uniform vs simulation-only vs two-phase vs ReLoBRaLo) on the same model; the current notebooks run a single persistent-simulation pipeline.}
\end{frame}
...
\midrule
\multicolumn{3}{l}{\textit{Irreversibility extension (notebook 03\_02 only):}} \\
...
\begin{itemize}
    ...
    \item FB residuals near zero $\Rightarrow$ complementarity is well satisfied (03\_02 only)
...
% --- Bridge frame ---
\begin{frame}[shrink=20]{Bridge: From IRBC to OLG and Beyond}
\textbf{What we leave with today.}  An IRBC DEQN trained with a single persistent-simulation pipeline and equal loss weights: the same recipe scales from $N=2$ to $N=10$ countries with no architectural change.  NAS-tuned width and ReLoBRaLo-balanced loss components are optional refinements, covered in this afternoon's tutorial (Session 2: NAS / loss normalization).

\vspace{0.4em}
\begin{columns}[T]
\column{0.48\textwidth}
\textbf{Next (Session 4): OLG with DEQNs.}
\begin{itemize}
    \item The analytic 6-cohort benchmark of Krueger--K\"ubler (2004) and the benchmark OLG economy of Azinovic, Gaegauf \& Scheidegger (2022).
    \item Borrowing constraints handled with a \emph{product-form} KKT residual ($\lambda \cdot g = 0$) --- a sibling of the Fischer--Burmeister trick introduced here.
\end{itemize}

\column{0.48\textwidth}
\textbf{Tomorrow morning (Day 3).}
\begin{itemize}
    \item Gaussian processes \& Bayesian numerical methods (Felix K\"ubler).
    \item Deep surrogate models \& deep uncertainty quantification --- with surrogate-based SMM and optimal carbon taxation as the worked applications.
\end{itemize}
\end{columns}
\end{frame}
