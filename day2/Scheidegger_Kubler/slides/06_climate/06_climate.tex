...
\subtitle{A baseline model of climate and the economy (DICE/CDICE), solved with Deep Equilibrium Nets}
...
\vspace{-0.55cm}
\begin{center}
{\footnotesize Session 6 --- the baseline climate--economy model, solved with DEQNs.}\\[4pt]
{\footnotesize Tuned production code: \url{https://github.com/ClimateChangeEcon/Climate_in_Climate_Economics}}\\[4pt]
...
% --- Frame 2: Roadmap ---
\begin{frame}{Where This Session Sits}
\begin{center}
\begin{tikzpicture}[
    box/.style={rectangle, draw=uzhblue, fill=uzhgreylight, rounded corners=4pt,
                minimum width=4.4cm, minimum height=1.5cm, align=center, font=\small},
    arr/.style={-{Stealth[length=2.8mm]}, very thick, uzhblue!75}
]
\node[box, fill=softblue!25] (here) at (0,0)
      {\textbf{This session (Day 2)}\\Baseline model of climate\\and the economy:\\DICE/CDICE $+$ DEQN};
\node[box] (day3) at (6.2,0)
      {\textbf{Day 3 (K\"ubler--Scheidegger)}\\Climate \emph{economics}:\\heterogeneous agents,\\constrained-optimal carbon taxes};
\draw[arr] (here) -- (day3);
\end{tikzpicture}
\end{center}

\vspace{0.2cm}
{\footnotesize Today we build and solve the \emphc{baseline climate--economy model}. The deeper
climate \emph{economics} --- optimal carbon taxation with heterogeneous agents ---
follows on \emphc{Day 3}.}

\vspace{0.2cm}
\textbf{In this session}:
\begin{enumerate}
    \item Motivation: why an integrated assessment model (IAM)?
    \item The DICE/CDICE model of climate and the economy
    \item Solving the deterministic baseline with a Deep Equilibrium Net
    \item A stochastic extension, and a look at deep uncertainty quantification
\end{enumerate}
\end{frame}
...
\vspace{2pt}
{\scriptsize\emphc{In this session's notebooks:} \texttt{06\_02}/\texttt{06\_03} solve the \emphc{8-equation CDICE core} (the first eight rows); the two Bayesian ECS-learning equations ($\mu_f, S_f$) are the Friedl et al.\ research extension, not implemented here.}
\end{frame}
...
\begin{frame}[shrink=5]{Epstein--Zin + i.i.d.\ weather (literature): model ingredients}
\small
\begin{alertblock}{Literature / preview --- not a notebook in this session}
\footnotesize The Epstein--Zin $+$ i.i.d.-weather formulation in the next four slides follows Friedl et al.\ (2023) and is shown as an advanced target. This session's stochastic notebook (\texttt{06\_03}) implements the simpler \emphc{CRRA $+$ AR(1)} version (Cai--Lontzek spirit); EZ with tipping risk is developed further in the full course.
\end{alertblock}
...
\begin{frame}[shrink=5]{Freezing trick (literature): \texttt{stop\_gradient} on EZ corrections}
\footnotesize
...
\begin{frame}[shrink=5]{$\gamma$-curriculum and diagnostics (literature)}
\footnotesize
...
Symmetric iid Gaussian weather $\Rightarrow$ EZ precautionary tilt on SCC is small (a few \%). The Cai--Lontzek ``EZ triples SCC'' result needs \emph{skewed} risk (tipping points) --- see the OLG-IAM with stochastic tipping in the full course.
\end{exampleblock}
\vspace{-0.1cm}
\end{frame}
...
\begin{frame}{Key Takeaways from This Session}
\begin{enumerate}
...
\vspace{0.3cm}
\begin{block}{Next}
Day 3 (K\"ubler--Scheidegger): extend from representative-agent IAM to \emphc{OLG with heterogeneous agents} $\rightarrow$ constrained Pareto-improving carbon tax rules.
\end{block}
\end{frame}

% --- Frame 38b: Notebook roadmap ---
\begin{frame}{Notebook Roadmap for This Session}
\small
The companion notebooks for this lecture sit in \texttt{code/06\_climate/} and are intended to be opened in this order:
\vspace{0.2cm}
\begin{enumerate}
    \item \texttt{06\_01\_Climate\_Exercise.ipynb} -- pure NumPy warm-up. Three-box carbon cycle, single-layer temperature, quadratic damages, BAU vs 50\% abatement. No neural networks: builds intuition for the climate module before the DEQN.
    \item \texttt{06\_02\_DICE\_DEQN\_Library\_Port.ipynb} -- deterministic CDICE solved with DEQN. Self-contained port of the production \texttt{DEQN\_for\_IAMs/dice\_generic} library, with the Friedl $\tau$-transformation and a verification gate against the reference solution.
    \item \texttt{06\_03\_Stochastic\_DICE\_DEQN.ipynb} -- (preview / self-study extension) AR(1) productivity shock, Gauss--Hermite quadrature on the Euler expectation, Monte-Carlo SCC fan charts in the Cai--Lontzek spirit.
\end{enumerate}
\end{frame}
