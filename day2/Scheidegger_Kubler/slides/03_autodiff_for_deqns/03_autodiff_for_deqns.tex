% ============================================================================
% Automatic Differentiation for Deep Equilibrium Nets
% Duration: ~50 minutes
% ============================================================================
...
\subtitle{Automatic differentiation for DEQNs: from the Lagrangian primitive to two-tape Euler residuals}
...
\vspace{-0.4cm}
\begin{center}
{\footnotesize Session 3 --- a primer for the autodiff notebooks (\texttt{03\_02\_Brock\_Mirman\_AutoDiff\_DEQN}, \texttt{03\_03\_Brock\_Mirman\_Uncertainty\_AutoDiff\_DEQN}).}\\[4pt]
...
\begin{enumerate}\itemsep4pt
  ...
  \item \textbf{Brock--Mirman by autodiff.} The one-line Euler residual; notebooks 02, 03.
...
\begin{itemize}
  \item Implement automatic differentiation to compute exact first and second derivatives of arbitrary code for economic models
  \item Compare symbolic, finite-difference, and autodiff approaches and diagnose failure modes in each
  \item Derive first-order conditions and envelope-theorem expressions using autodiff instead of hand algebra
  \item Build a two-tape computation graph to compute Euler residuals directly from the Lagrangian
  \item Deploy gradient-descent optimization on neural-network policies using exact gradients computed via reverse-mode autodiff
\end{itemize}
\end{frame}
...
\begin{frame}[shrink=22]{Derivatives are \emph{everywhere} in macro-finance}
\begin{columns}[t]
\column{0.5\textwidth}
...
\item \textbf{Sensitivities and uncertainty quantification.} $\partial (\text{policy})/\partial (\text{parameter})$ for robustness, comparative statics, and deep UQ (Day~8).
\end{enumerate}
\vspace{4pt}
...
\footnotesize The original hand-derived Brock--Mirman notebook (\texttt{lecture\_03\_01\_Brock\_Mirman\_1972\_DEQN}; the autodiff version is \texttt{lecture\_07\_02\_Brock\_Mirman\_AutoDiff\_DEQN}) needed \emph{two pages} of algebra before we could code the loss:
\begin{center}
...
\end{tikzpicture}
\end{center}
Notebook~02 (same model, autodiff) collapses all four red boxes into one function:
\begin{center}
\begin{tikzpicture}[node distance=4mm]
...
\begin{frame}[shrink=12]{Method 3: automatic differentiation -- the elevator pitch}
\small
...
\begin{frame}[shrink=20]{Where do the local derivatives come from? A tiny built-in table}
\footnotesize
...
\begin{frame}[shrink=10]{Forward mode: worked example on $y = x^2 + \sin(x)$ at $x=2$}
\footnotesize
...
\small Convex adjustment cost, from Day~3's IRBC model:
\[
...
\begin{frame}[shrink=10]{The deterministic Brock--Mirman model}
...
\item This is the equation that \texttt{compute\_cost} in the hand-derived Brock--Mirman notebook (\texttt{lecture\_03\_01}) minimises the squared residual of.
\end{itemize}
\end{frame}
...
\begin{frame}[shrink=18]{Recap: what we derived by hand}
\small
\begin{center}
...
\textbf{Hand-derived (\texttt{lecture\_03\_01})}
...
\column{0.49\textwidth}
\textbf{Autodiff (\texttt{lecture\_07\_02})}
\begin{lstlisting}[language=Python,basicstyle=\ttfamily\tiny]
def Pi(K_in, K_out):
    Y = K_in**alpha
...
\small The stochastic Brock--Mirman model (autodiff version: notebook~03) adds an AR(1) shock $z_t$:
\[
...
\footnotesize From notebooks \texttt{03\_02\_Brock\_Mirman\_AutoDiff\_DEQN} and \texttt{03\_03\_Brock\_Mirman\_Uncertainty\_AutoDiff\_DEQN}:
\vspace{2pt}
\begin{center}
\renewcommand{\arraystretch}{1.15}
...
\footnotesize The autodiff loss is not an \emph{approximation} of the textbook Euler equation -- it \emph{is} the textbook Euler equation, computed by a different mechanism. Training works as well as the hand-derived Brock--Mirman notebooks. We simply stopped writing algebra.
\end{block}
\end{frame}
...
\item \textbf{OLG.} A separate $\Pi$ per age cohort; gradients stack.
\item \textbf{Irreversibility constraints} (IRBC). Fischer--Burmeister residuals sit alongside; the core Euler part still autodiffs.
\end{itemize}
\vspace{6pt}
...
\footnotesize
\begin{block}{Notebook \texttt{03\_04\_IRBC\_AutoDiff\_DEQN.ipynb}}
The Day~3 baseline IRBC model (multi-country, adjustment costs, irreversibility) re-solved using the same $\partial_2\Pi + \beta\,\mathbb{E}[\partial_1\Pi]$ template, scaled from one Brock--Mirman state to a multi-equation planner Lagrangian.
\end{block}
\vspace{2pt}
\begin{itemize}\itemsep2pt
...
\item \textbf{Cross-check 1.} Autodiff Euler residual versus the Day~3 hand-derived residual on the same untrained network: \emphc{$\le 1.2\times 10^{-7}$} (float32 epsilon).
...
\item \emphc{Fischer--Burmeister} for KKT complementarity (Day~3 IRBC).
...
\begin{itemize}\itemsep1pt
\item \emphc{Reparameterise} so $C > 0$ \emph{by architecture}. The Brock--Mirman DEQN notebooks use a sigmoid on the savings rate $s \in (0,1)$, guaranteeing $C > 0$ and $K' > 0$.
...
\textbf{Mitigations.}
\begin{itemize}\itemsep1pt
\item \emphc{Path averaging / mini-batching} over trajectory segments (the simulation-based Brock--Mirman notebook).
...
\item Cross-check on a simple case: the autodiff number should agree with the closed-form formula to machine precision (this is exactly what the autodiff Brock--Mirman notebooks~02 and~03 do).
\item Use AD to scale, not to replace, economic understanding.
\end{itemize}
\end{columns}
\end{frame}

\begin{frame}[fragile,shrink=15]{Pitfall 5: \texttt{stop\_gradient} and the envelope theorem}
...
\begin{frame}[shrink=15]{One slide to remember}
\small
...
\begin{frame}[fragile,shrink=15]{Where to go next}
\footnotesize
\begin{block}{Run the notebooks}
\begin{itemize}\itemsep1pt
\item \texttt{03\_01\_AutoDiff\_Analytical\_Examples}: warm-ups + 2-D gradient fields from \S 4.
\item \texttt{03\_02\_Brock\_Mirman\_AutoDiff\_DEQN}: deterministic Brock--Mirman.
\item \texttt{03\_03\_Brock\_Mirman\_Uncertainty\_AutoDiff\_DEQN}: stochastic Brock--Mirman + Gauss--Hermite.
\item \texttt{03\_04\_IRBC\_AutoDiff\_DEQN} (\emph{further example, self-study}): IRBC under the autodiff template -- multi-country planner Lagrangian, machine-precision cross-check vs the IRBC session (Session 4).
\end{itemize}
