...
%            notebook 04_04 (benchmark_persistent) implements
...
\subtitle{Overlapping generations, borrowing constraints, product-form KKT complementarity}
...
\vspace{-0.4cm}
\begin{center}
{\footnotesize Based on Azinovic, Gaegauf \& Scheidegger (2022, \emph{IER} 63(4), 1471--1525) and Krueger \& K\"ubler (2004, \emph{JEDC}).}\\[8pt]
...
\vspace{-3pt}\begin{itemize}\itemsep0pt
    ...
    \item Primary notebook: \texttt{04\_02\_OLG\_Analytic\_DEQN\_persistent.ipynb}; companion ablation \texttt{04\_01\_OLG\_Analytic\_DEQN\_exogenous.ipynb}.
\end{itemize}
\end{block}
\vspace{-2pt}
...
\vspace{-3pt}\begin{itemize}\itemsep0pt
    ...
    \item Primary notebook: \texttt{04\_04\_OLG\_Benchmark\_DEQN\_persistent.ipynb}; companion ablation \texttt{04\_03\_OLG\_Benchmark\_DEQN\_exogenous.ipynb}.
\end{itemize}
\end{block}
\end{frame}
...
\scriptsize
\emphc{Open notebook \texttt{04\_02\_OLG\_Analytic\_DEQN\_persistent.ipynb}} (the persistent-simulation primary; the exogenous-sampling companion is \texttt{04\_01\_OLG\_Analytic\_DEQN\_exogenous.ipynb}).  It implements the model, equilibrium conditions, loss, FREE map, architecture, training, and validation covered in Part II.
\vspace{-2pt}
...
\vspace{2pt}
The actual training run, DEQN-vs-analytic comparison, and Euler-error histogram are produced by \texttt{04\_02\_OLG\_Analytic\_DEQN\_persistent.ipynb} (\S 8).  Production target in AGS (2022): mean relative Euler error $\lesssim 10^{-3}$.
\end{block}
\end{frame}
...
Open \texttt{04\_02\_OLG\_Analytic\_DEQN\_persistent.ipynb}.  Set \texttt{RUN\_MODE = "teaching"} ($\sim 5$ min) for the classroom run; \texttt{"smoke"} (the default) is a $\sim$30-s sanity check; \texttt{"production"} chases AGS Table~3 accuracy.  Both notebooks expose a \texttt{SAMPLING\_MODE = "simulation" / "exogenous"} switch (the persistent primary uses \texttt{"simulation"}, the companion \texttt{04\_01\_OLG\_Analytic\_DEQN\_exogenous.ipynb} uses \texttt{"exogenous"}); the model and loss are identical -- only the training cloud differs.
\end{block}
\end{frame}


% =====================================================================
% Part III -- Benchmark (AGS 2022 leading example), notebook 04_04 (benchmark_persistent)
% =====================================================================

\section{Part III: Benchmark 56-cohort OLG (AGS 2022 \S 3)}

% --- III.1 Setup -------------------------------------------------------
\begin{frame}[shrink=6]{III.1 Setup --- the Leading Example of AGS (2022)}
\small
\begin{columns}[T]
...
\vspace{0.2em}
{\scriptsize \emphc{Notation vs.\ Session 3 (IRBC):} here $\alpha$ = capital share ($\zeta$ there), $\zeta$ = adjustment-cost coefficient ($\kappa$ there), $\kappa$ = collateral parameter, and $\gamma$ = CRRA risk aversion (IES there).}
...
\]
{\scriptsize Uniform weights across $4(A-1)+1$ residuals of different scales; \emph{balancing} such components is exactly the topic of this afternoon's loss-normalization tutorial.}
...
\end{bmatrix}^{\!\top}
\]
{\footnotesize Output blocks: capital savings; borrowing multipliers; bond holdings $\hat b'^h$ (transformed to satisfy the collateral floor $\hat k'^h + \kappa\,\hat b'^h \ge 0$); collateral multipliers $\hat\mu^h$ ($=\lambda_c$ above); equilibrium bond price (one scalar).}
...
\begin{block}{Sizes from \texttt{04\_04\_OLG\_Benchmark\_DEQN\_persistent.ipynb}}
\centering
\begin{tabular}{ll}
\toprule
...
\vspace{-3pt}
For each aggregate state $z \in \{1,2,3,4\}$ the trained policy yields a lifecycle consumption profile $c^s(z)$ over ages $s=25,\ldots,80$, read off a long ergodic simulation from \texttt{04\_04\_OLG\_Benchmark\_DEQN\_persistent.ipynb}.  (The notebook ships pooled lifecycle bands and residual diagnostics; the per-shock breakdown is a small add.)
\end{block}
...
\vspace{-2pt}
\textbf{Hands-on: \texttt{04\_04\_OLG\_Benchmark\_DEQN\_persistent.ipynb}} (persistent-simulation primary; \texttt{04\_03\_OLG\_Benchmark\_DEQN\_exogenous.ipynb} is the feedback-free ablation, same model and loss, different training cloud).  \S 1--4 match III.1--III.5; \S 5--7 = training and policy stability; \S 8 = plots/diagnostics of III.10--III.12.  Teaching runtime ($\sim$200 segments, 128-wide): a few minutes.
\end{frame}
...
\vspace{0.2em}
{\footnotesize \textbf{Hands-on exercise:} \texttt{04\_05\_OLG\_Exercise.ipynb} --- guided tasks on the analytic 6-agent model, building on notebook 04\_02.}
\end{frame}
...
\begin{itemize}\itemsep3pt
    \item Azinovic, M., Gaegauf, L., \& Scheidegger, S. (2022). \emph{Deep equilibrium nets.} International Economic Review 63(4), 1471--1525.  \;[\,notebook 04\_04 (\texttt{benchmark\_persistent}) implements \S 3 of this paper].

    \item Krueger, D., \& K\"ubler, F. (2004). \emph{Computing equilibrium in OLG models with stochastic production.} Journal of Economic Dynamics \& Control 28(7), 1411--1436.  \;[\,notebook 04\_02 (\texttt{analytic\_persistent}) implements their 6-agent analytic model].

    \item Fischer, A. (1992). \emph{A special Newton-type optimization method.} Optimization 24, 269--284.  \;[FB-complementarity function -- used in \textbf{Session 3 (IRBC)}; mentioned in this lecture only as an alternative to the product-form KKT residuals adopted here.]
...
\vspace{0.4em}
\textbf{Coming up tomorrow morning (Day 3):} Gaussian processes \& Bayesian numerical methods (Felix K\"ubler), then deep surrogate models \& deep uncertainty quantification --- with surrogate-based SMM and optimal carbon taxation as the worked applications.  \emph{Beyond this course:} heterogeneous agents on a histogram (Young's method), continuum-of-agents DEQN, Krusell--Smith with deep learning, and the sequence-space reformulation.
\end{frame}
