...
\subtitle{Random search and Hyperband for DEQN architectures}
...
\begin{center}
{\scriptsize Hands-on:
\texttt{02\_01\_NAS\_Random\_Search\_10D.ipynb} (10-D random search)\\
\texttt{02\_02\_NAS\_RandomSearch\_Hyperband.ipynb} (Random Search + SHA)}
\end{center}
\end{frame}
...
\vspace{0.25cm}
{\scriptsize\itshape Illustrative numbers from a depth/width sweep on the companion regression task; the from-scratch search in Notebook~02\_02 reproduces the same ordering on a different testbed.}
...
\begin{itemize}
    \item Learning rate $\mathrm{lr}$ log-uniform on $[10^{-4}, 10^{-2}]$ (20-point grid)
...
\begin{tikzpicture}[scale=0.85]
    \draw[->] (0,0) -- (4.5,0) node[right] {$\mathrm{lr}$};
...
\begin{itemize}\itemsep0pt
    ...
    \item No need for 200 epochs to know $\mathrm{lr}\!=\!10$ diverges
...
\vspace{0.3cm}
\textbf{Baseline:} $3 \times 64$ ReLU MLP, $\mathrm{lr} = 10^{-3}$
...
\vspace{-0.4em}
{\scriptsize Generated by \texttt{02\_02\_NAS\_RandomSearch\_Hyperband.ipynb}.  Random Search uses $30$ full trials; SHA uses $27$ initial candidates with $\eta=3$.  Both reach MAE of order $10^{-3}$ from a baseline near $4\cdot10^{-3}$; SHA uses about $2.3\times$ less config-epoch budget in this seeded run.}
\end{frame}
...
\vspace{-0.2em}
{\footnotesize Generated by \texttt{02\_02\_NAS\_RandomSearch\_Hyperband.ipynb}.  Left to right: the true Genz target, the best NAS-found NN approximation, and the absolute error map on $[0,1]^2$.  No placeholder shapes; this is the actual notebook output.  The companion \texttt{02\_01\_NAS\_Random\_Search\_10D.ipynb} offers a 10-D testbed (search loop in plain Python, model in TF/Keras).}
\end{frame}
...
\begin{enumerate}
    ...
    \vspace{0.2cm}
    \item \textbf{DSGE policy functions:} Deep equilibrium nets (Session 1) use NNs to approximate policy/value functions. \emphc{Architecture search over the policy network} can improve convergence.
...
\vspace{0.4em}
{\small \emphc{Up next:} we tuned the \emph{architecture}; the second half of this session balances the \emph{multi-component residual loss} (ReLoBRaLo \& friends).}
