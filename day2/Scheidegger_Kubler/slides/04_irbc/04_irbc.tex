...
\begin{enumerate}
    ...
    \item Open notebook \texttt{04\_01\_IRBC\_DEQN\_smooth.ipynb}, change $\delta$, and verify your prediction.
\end{enumerate}
\end{exampleblock}
\end{frame}
...
\vspace{0.15em}
\textbf{Smooth benchmark} (notebook 04\_01):
\[
\boxed{
...
\vspace{0.1em}
\textbf{Irreversibility extension} (notebook 04\_02): add the Fischer--Burmeister block,
\[
...
\begin{itemize}\itemsep0pt
    ...
    \item $\text{FB}_j = \mu^j + I^j - \sqrt{(\mu^j)^2 + (I^j)^2 + \varepsilon^2}$ \quad (\emph{04\_02 only})
\end{itemize}
\end{frame}
...
\vspace{0.2em}
{\footnotesize \textbf{What the companion notebooks do.} Both 04\_01 and 04\_02 use the \emphc{Stroud-3 monomial rule} ($2(N+1)$ nodes; $6$ for $N=2$), not the tensor-product Gauss--Hermite above; the latter is shown for contrast and is practical only at very low $N$.}
\end{frame}
...
\footnotesize
\textbf{Three notebooks accompany this lecture} (all in \texttt{code/04\_irbc/}; self-contained, all equations inline):

\vspace{0.2em}
\begin{enumerate}\itemsep1pt
    \item \texttt{04\_01\_IRBC\_DEQN\_smooth.ipynb} -- \emph{smooth benchmark, in-class walk-through}
    \begin{itemize}\itemsep0pt
    ...
    \item \texttt{04\_02\_IRBC\_DEQN\_irreversible.ipynb} -- \emph{irreversible-investment extension}
    \begin{itemize}\itemsep0pt
    ...
    \item \texttt{04\_03\_IRBC\_Exercise.ipynb} -- \emph{hands-on exercise; uses a \emph{standalone} quarterly calibration ($\delta=0.025$, $A_{\mathrm{tfp}}=1$), deliberately decoupled from the DEQN notebooks' $k_{ss}=1$ normalization}
    \begin{itemize}\itemsep0pt
...
\begin{itemize}
    ...
    \begin{itemize}\itemsep1pt
        \item smooth (04\_01): $k^{j\prime} = k^j \exp\{\bar g \tanh r_j(\bm{s})\}$ keeps $k^{j\prime} > 0$;
        \item irreversible (04\_02): $k^{j\prime} = (1-\delta)k^j + I^j$ with $I^j \ge 0$ enforced via a sigmoid head.
    \end{itemize}
...
\begin{itemize}
    ...
    \item In a sensible SSS: $I^j \approx \delta\,k^j$ (investment replaces depreciation) and, in 04\_02, the irreversibility multiplier $\mu^j \approx 0$.
\end{itemize}
\end{frame}
...
\begin{enumerate}\itemsep3pt
    ...
    \item \textbf{Per-residual mean magnitudes} -- \texttt{mean\_abs\_euler}, \texttt{mean\_abs\_arc}, and (in 04\_02) \texttt{mean\_abs\_fb} -- all fall below the prescribed tolerance.
    ...
    \item \textbf{Zero-shock SSS} converges from dispersed feasible starts to a common point with $I^j \to \delta\,k^j$ and (in 04\_02) $\mu^j \to 0$.
\end{enumerate}
...
\midrule
\multicolumn{3}{l}{\textit{Irreversibility extension (notebook 04\_02 only):}} \\
...
\begin{itemize}
    ...
    \item FB residuals near zero $\Rightarrow$ complementarity is well satisfied (04\_02 only)
...
\begin{columns}[T]
\column{0.48\textwidth}
\textbf{Next (Session 5): OLG with DEQNs.}
\begin{itemize}
